...
(function (r, j) {
  ...
  var A,
    m = r.performance;
  A = m
    ? m[m.now ? "now" : "webkitNow"].bind(m)
    : function () {
        return +new Date();
      };
  for (
    var C = r.cancelAnimationFrame || r.cancelRequestAnimationFrame,
    ...
    M =
      (C =
        r[m[k] + "CancelAnimationFrame"] ||
        r[m[k] + "CancelRequestAnimationFrame"]) &&
      r[m[k] + "RequestAnimationFrame"];
  C ||
    ((M = function (a) {
      var e = A(),
        g = Math.max(0, 16 - (e - G));
